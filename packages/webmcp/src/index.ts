...
const SCHEMAS: Record<WebMcpToolName, JsonObject> = {
  ...
  request_quote: { type: 'object', properties: { productId: { type: 'string', minLength: 1, maxLength: 120, description: 'The active storefront product requiring merchant review.' }, quantity: { type: 'integer', minimum: 1, maximum: 99999, description: 'Total requested quantity. The size breakdown must total this value.' }, configuration: { type: 'object', description: 'Supported custom-product configuration. Required by the controlled TheCustomHub shirt fixture.', properties: { color: { type: 'string', minLength: 1, maxLength: 60, description: 'Requested supported garment color.' }, sizeBreakdown: { type: 'array', minItems: 1, maxItems: 20, description: 'Unique supported sizes whose quantities total the requested quantity.', items: { type: 'object', properties: { size: { type: 'string', enum: ['Youth Small', 'Youth Medium', 'Youth Large'] }, quantity: { type: 'integer', minimum: 1, maximum: 99999 } }, required: ['size', 'quantity'], additionalProperties: false } }, personalization: { type: 'object', properties: { placement: { type: 'string', enum: ['Front'] }, method: { type: 'string', enum: ['One-color robotics-team logo'] } }, required: ['placement', 'method'], additionalProperties: false }, artworkStatus: { type: 'string', enum: ['To be supplied after quote request'] } }, required: ['color', 'sizeBreakdown', 'personalization', 'artworkStatus'], additionalProperties: false }, requestedDelivery: { type: 'object', description: 'Shopper request, not a merchant delivery promise.', properties: { destination: { type: 'string', minLength: 1, maxLength: 160 }, requestedWithinDays: { type: 'integer', minimum: 1, maximum: 365, description: 'Requested relative delivery window; not a merchant promise.' } }, required: ['destination', 'requestedWithinDays'], additionalProperties: false }, budget: { type: 'object', description: 'Shopper budget ceiling, not a merchant price.', properties: { amount: { type: 'number', exclusiveMinimum: 0, maximum: 100000 }, currency: { type: 'string', minLength: 3, maxLength: 3 }, isMaximum: { type: 'boolean', description: 'Must be true when this is a shopper budget ceiling.' } }, required: ['amount', 'currency', 'isMaximum'], additionalProperties: false }, requirements: { type: 'string', minLength: 1, maxLength: 2000, description: 'Bounded merchant-review notes not represented above.' }, idempotencyKey: { type: 'string', minLength: 8, maxLength: 160 } }, required: ['quantity', 'requirements', 'idempotencyKey'], additionalProperties: false },
...
export function createRetailAgentWebMcp(input: { gateway: RetailAgentGateway; storefront: StorefrontBridge; adapter?: WebMcpBrowserAdapter; clock?: () => number; telemetry?: (event: WebMcpTelemetryEvent) => void; enableCartRevision?: boolean }) {
  ...
  function makeDescriptors(): Record<WebMcpToolName, WebMcpToolDescriptor> {
    ...
    return {
      ...
      request_quote: descriptor('request_quote', 'Request merchant quote', 'Submit structured custom-product requirements for merchant review when RetailAgentOS determines that no valid fixed price exists. Include quantity, supported configuration, requested delivery window, shopper budget ceiling, and bounded notes. The size breakdown must total the quantity. Delivery is not a merchant promise and the budget is not a price. This tool creates no cart, order, payment, or checkout.', { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false, untrustedContentHint: false }, async (args, signal) => { const result = await input.gateway.requestQuote({ productId: optionalString(args, 'productId', 120), quantity: requiredInt(args, 'quantity', 99999), configuration: quoteConfigurationArg(args), requestedDelivery: quoteDeliveryArg(args), budget: quoteBudgetArg(args), requirements: stringArg(args, 'requirements', 2000), idempotencyKey: stringArg(args, 'idempotencyKey', 160) }, { signal }); input.storefront.onQuote?.(result); return { result: result as unknown as JsonObject, nextState: 'quote_requested' as const }; }),
...
function errorResult(code: string): JsonObject & { code: string; message: string; nextAction: string; retryable: boolean } {
  const guidance: Record<string, { message: string; nextAction: string; retryable: boolean }> = {
    ...
    MISSING_CONFIGURATION: { message: 'This custom product needs a supported configuration.', nextAction: 'Provide color, size breakdown, personalization, and artwork status.', retryable: true },
    UNSUPPORTED_COLOR: { message: 'The requested color is not supported.', nextAction: 'Use a color returned by search_catalog.', retryable: true },
    UNSUPPORTED_SIZE: { message: 'The requested size is not supported.', nextAction: 'Use supported sizes from the catalog configuration options.', retryable: true },
    DUPLICATE_SIZE: { message: 'A size was supplied more than once.', nextAction: 'Provide one line per size.', retryable: true },
    SIZE_QUANTITY_MISMATCH: { message: 'The size quantities do not total the requested quantity.', nextAction: 'Correct the size breakdown so it totals the quote quantity.', retryable: true },
    UNSUPPORTED_PERSONALIZATION: { message: 'The requested personalization is not supported.', nextAction: 'Use the supported placement and method from the catalog.', retryable: true },
    INVALID_ARTWORK_STATUS: { message: 'The artwork status is not supported.', nextAction: 'Use the supported artwork status from the catalog.', retryable: true },
    INVALID_DELIVERY_REQUEST: { message: 'The delivery request is not valid.', nextAction: 'Provide a destination and a requested window from 1 to 365 days.', retryable: true },
    INVALID_BUDGET: { message: 'The shopper budget ceiling is not valid.', nextAction: 'Provide a positive three-letter-currency budget with isMaximum true.', retryable: true },
...
function objectArg(args: JsonObject, key: string) { const value = args[key]; if (value === undefined) return undefined; if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('INVALID_INPUT'); return value as JsonObject; }
function quoteConfigurationArg(args: JsonObject) { const value = objectArg(args, 'configuration'); if (!value) return undefined; const sizes = value.sizeBreakdown; if (!Array.isArray(sizes) || !sizes.length || sizes.length > 20) throw new Error('INVALID_INPUT'); const personalization = objectArg(value, 'personalization'); if (!personalization) throw new Error('INVALID_INPUT'); return { color: stringArg(value, 'color', 60), sizeBreakdown: sizes.map((line) => { if (!line || typeof line !== 'object' || Array.isArray(line)) throw new Error('INVALID_INPUT'); const entry = line as JsonObject; return { size: stringArg(entry, 'size', 60), quantity: requiredInt(entry, 'quantity', 99999) }; }), personalization: { placement: stringArg(personalization, 'placement', 60), method: stringArg(personalization, 'method', 120) }, artworkStatus: stringArg(value, 'artworkStatus', 120) }; }
function quoteDeliveryArg(args: JsonObject) { const value = objectArg(args, 'requestedDelivery'); return value ? { destination: stringArg(value, 'destination', 160), requestedWithinDays: requiredInt(value, 'requestedWithinDays', 365) } : undefined; }
function quoteBudgetArg(args: JsonObject) { const value = objectArg(args, 'budget'); if (!value) return undefined; const amount = value.amount; if (typeof amount !== 'number' || !Number.isFinite(amount) || amount <= 0 || amount > 100000 || typeof value.isMaximum !== 'boolean') throw new Error('INVALID_INPUT'); return { amount, currency: stringArg(value, 'currency', 3), isMaximum: value.isMaximum }; }
