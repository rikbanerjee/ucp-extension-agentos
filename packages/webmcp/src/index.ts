...
import type { JsonObject, PlanDecision, RetailAgentGateway, ShowcaseState, StorefrontBridge, WebMcpBrowserAdapter, WebMcpRegistration, WebMcpTelemetryEvent, WebMcpToolAnnotations, WebMcpToolDescriptor, WebMcpToolName } from './types';

export const CANONICAL_TOOLS: WebMcpToolName[] = ['get_storefront_capabilities', 'search_catalog', 'evaluate_shopping_plan', 'find_valid_alternatives', 'apply_plan_repair', 'prepare_validated_cart', 'request_quote'];
const BASE_TOOLS = CANONICAL_TOOLS.slice(0, 3);
...
export function getToolsForState(state: ShowcaseState): WebMcpToolName[] { return [...BASE_TOOLS, ...PHASE_TOOLS[state]]; }

export function createRetailAgentWebMcp(input: { gateway: RetailAgentGateway; storefront: StorefrontBridge; adapter?: WebMcpBrowserAdapter; clock?: () => number; telemetry?: (event: WebMcpTelemetryEvent) => void }) {
  ...
  function names() { return modelContext ? [...activeTools] : getToolsForState(state); }
  async function transition(nextState: ShowcaseState) {
    const previous = state; const oldTools = PHASE_TOOLS[state]; const newTools = PHASE_TOOLS[nextState];
    ...
    if (!modelContext) return getToolsForState(state);
  ...
  async function register(): Promise<WebMcpRegistration> {
    ...
    return { supported: Boolean(modelContext), get registeredTools() { return modelContext ? [...activeTools] : []; }, getReplayTools: () => getToolsForState(state), invoke: async (name, args, signal) => { const result = await descriptors[name].execute(args, { signal, source: 'replay' }); await settleRegistry(); return result; }, setState: transition, settleRegistry, getNativeToolNames: async () => { if (!modelContext?.getTools) return []; const observed = await modelContext.getTools(); return observed.map((tool) => typeof tool === 'string' ? tool : tool.name).filter((tool): tool is WebMcpToolName => CANONICAL_TOOLS.includes(tool as WebMcpToolName)); }, dispose: () => { if (disposed) return; disposed = true; const previous = state; const src = registrationSource(); sessionController.abort(); phaseController?.abort(); baseController?.abort(); [...activeTools].forEach((tool) => emit(tool, 'unregistered', src, { previousState: previous, nextState: 'initial' })); activeTools.clear(); state = 'initial'; } };
  }
