...
- Historical provenance matters: native browser WebMCP is shipped for the controlled showcase; the generalized remote/server MCP is designed, not shipped. Keep pre-challenge foundations distinct from challenge-period commits `92753e5`, `d094e12`, `e464bb8` (Aug 30–31, 2026), `d9a5eb5` (judge-facing UX/truthfulness), `0228160` (optional cart-revision extension), and the submission-hardening pass documented in `specs/WEBMCP-PLATFORM-BUILD.md`'s "Submission-hardening pass" section — and update public agent, evidence, developer, and build-log surfaces together when changing this terminology.
...
- The shopper-approval card (`ShopperApprovalCard.tsx`) shows a completed "Approved by shopper" state, not a vanished card, followed by a real telemetry-attributed sequence (WebMCP lifecycle registration → RetailAgentOS validation → browser agent/guided replay invocation). Never label a UI click as a WebMCP invocation.
